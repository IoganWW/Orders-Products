...
interface OrderDetailsProps {
  order: Order;
  onClose: () => void;
  onDeleteProduct: (orderId: number, productId: number) => void;
  onAddProduct: (orderId: number) => void;
}

// Обновленный интерфейс для OrderProductItem
const OrderProductItem: React.FC<{ product: Product, orderId: number, onDeleteProduct: (orderId: number, productId: number) => void }> = ({ product, orderId, onDeleteProduct }) => {
  ...
  return (
    <div className={`${styles.orderDetails__productItem}`}>
      ...
      <button
        ...
        onClick={() => onDeleteProduct(orderId, product.id)}
